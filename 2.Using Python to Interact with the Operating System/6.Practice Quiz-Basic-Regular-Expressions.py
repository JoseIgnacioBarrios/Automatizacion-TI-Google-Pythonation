...
and a plus sign, followed by a period and a character-only top-level domain such as ".com", ".info", ".edu", etc. Fill 
in the regular expression to do that, using escape characters, wildcards, repetition qualifiers, beginning and end-of-line 
...
# the hour is between 1 and 12, with no leading zero, followed by a colon, then minutes
 # between 00 and 59, then an optional space, and then AM or PM, in upper or lower 
 # case. Fill in the regular expression to do that. How many of the concepts that you 
 # just learned can you use here?

import re
def check_time(text):
  pattern = r"^[1-9][0-9]|^[1-9]:[0-5][0-9]( ?)[AapP][Mm]"
  result = re.search(pattern, text)
  return result != None
...
print(contains_acronym("Please do NOT enter without permission!")) # Fals
...
# Pregunta 4
# What does the "r" before the pattern 
# string in re.search(r"Py.*n", sample.txt) indicate?

# Raw strings

# Correcto
# Right on! "Raw" strings just means the Python interpreter won't try to 
# interpret any special characters and, instead, will just pass the string 
# to the function as it is. 
...
# Correcto
# Awesome! The plus character [+], matches one or more occurrences of the 
# character that comes before it. 

# Pregunta 6
# Fill in the code to check if the text passed includes a possible U.S. zip code, 
# formatted as follows: exactly 5 digits, and sometimes, but not always, followed by 
# a dash with 4 more digits. The zip code needs to be preceded by at least one space, 
# and cannot be at the start of the text.

import re
def check_zip_code (text):
  result = re.search(r" [0-9]{5}|[0-9]{5}-[0-9]{4}?", text)
  return result != None
